...
class GSpreadGateway:
    ...
    SHEET_NAME = 'Sheet1'
    FIND_TERM = 'Category'
    # have also included NHS Number, 'Date Updated', 'Date Time Extracted' as would be useful for data warehouse etc
    COLS = ['Account ID', 'NHS Number', 'Forename', 'Surname', 'Gender', 'Date of Birth', 'House Number', 'Postcode',
            'Email', 'Phone', 'Phone2', 'First Symptomatic At', 'Date Tested', 'Comments', 'Date Updated',
            'Date Time Extracted']
    ...
    def __init__(self, key_file_location, google_drive_gateway):
        ...
        self.gspread_service = gspread.authorize(creds)  # makes the gspread service

    def get_cases_from_gsheet(self, sheet_key):
        print("[get_cases_from_gsheet] Trying to open by key using %s" % sheet_key)
        spreadsheet = self.gspread_service.open_by_key(sheet_key)
        print(spreadsheet)
        sheet = spreadsheet.worksheet(self.SHEET_NAME)
        # find first cell in T&T dowbnload - this should be 'Category', and use returned row value to get starting point of data
        first_cell = sheet.find(self.FIND_TERM)
        df = gspread_dataframe.get_as_dataframe(sheet, skiprows=2, parse_dates=True)
    ...
    def clean_data(self, df): # takes whole sheet and lints data
        ...
        df['Account ID'] = df['Account ID'].astype(str) # some account ids are just numbers, we need them to be string
        return df
    ...
    def create_output_spreadsheet(self, data_frame, outbound_folder_id):
        ...
        # create a new spreadsheet TODO this only writes to personal drive so need way to write to shared drive - service account better for this?

        sh = self.gspread_service.open_by_key(
            self.google_drive_gateway.create_spreadsheet(outbound_folder_id, new_spreadsheet_name)
        )
        print(sh)
    ...
    def populate_output_sheets(self, spreadsheet, df, title):
        ...
        worksheet = spreadsheet.add_worksheet(title=title, rows=df.shape[0] + 10, cols=len(self.COLS))
        next_row = self.next_available_row(worksheet)
        gspread_dataframe.set_with_dataframe(worksheet, df, include_column_header=True, row=int(next_row), col=1)
