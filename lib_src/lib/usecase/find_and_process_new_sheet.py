...
class FindAndProcessNewSheet:
    ...
    def execute(self, inbound_folder_id, outbound_folder_id):
        ...
        if self.google_drive_gateway.searchFolder(
                inbound_folder_id, today, "spreadsheet"):
            if self.google_drive_gateway.searchFolder(
                    outbound_folder_id, today, "spreadsheet") == False:
                print(
                    "[CheckLogic: %s] Run DataWrangleScript" %
                    dt.datetime.now())
                foundFileID = self.google_drive_gateway.get_file(
                    inbound_folder_id, today, "spreadsheet")
                print(foundFileID)

                df = self.gspread_drive_gateway.get_cases_from_gsheet(
                    foundFileID)
    ...
    def get_hackney_cases(self, df):
        ...
        hack_df = hack_df[(~hack_df['Phone'].isna()) |
                          (~hack_df['Phone2'].isna())]
    ...
    def get_text_message_list(self, df):
        ...
        text_list = df[~df['Phone'].isna()]
        text_list = text_list[['Phone']]
        # print(text_list)
        return text_list

    # Only returns cases that don't have any phone number
    def get_address_email_lists(self, df):
        print(
            "[get_address_email_lists] Only returns cases that don't have any phone number")
        address_list = df[(df['House Number'].str.len() > 0) & (df['Postcode'].str.len(
        ) > 0) & (df['Phone'].str.len() == 0) & (df['Phone2'].str.len() == 0)]
        address_list = address_list[[
            'Date Time Extracted', 'Forename', 'Surname', 'House Number', 'Postcode']]
        email_list = df[(df['Email'].str.len() > 0) & (
            df['Phone'].str.len() == 0) & (df['Phone2'].str.len() == 0)]
